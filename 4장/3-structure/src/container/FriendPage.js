...
export default function FriendPage() {
  ...
  const firendsWithNameLengethLimit = friends.filter(
  ...
  return (
    <div>
      ...
      <NumberSelect
        value={nameLenLimit}
        options={NAME_LIMIT_LEN_OPTIONS}
        label="자 이하 이름만 보기"
        onChange={setNameLenLimit}
      />
      <FriendList friends={firendsWithNameLengethLimit} />
    </div>
  );
}
...
const NAME_LIMIT_LEN_OPTIONS = [2, MAX_NAME_LEN_LIMIT];
